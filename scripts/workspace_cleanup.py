...
CONFIRMATION = "quarantine-expired-court4-workspaces"
WORKSPACE_PREFIX = "court4-"
...
def inspect_expired_workspaces(
    root: Path,
    *,
    ...
    for directory in sorted(
        ...
        if path.is_dir() and path.name.startswith(WORKSPACE_PREFIX)
    ):
...
def run_cleanup(
    ...
    *,
    ...
    resolved_root = root.expanduser().resolve()
    expired, total_files, total_bytes = inspect_expired_workspaces(
        resolved_root,
    ...
    if apply:
        if confirmation != CONFIRMATION:
            raise ValueError(
                "Cleanup refusal: exact workspace quarantine confirmation is required."
            )
        ...
        quarantine_root = resolved_root / "_quarantine" / "workspaces"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Dry-run-first Court4 workspace cleanup.")
    parser.add_argument("--root", type=Path, default=settings.processing_workspace_root)
